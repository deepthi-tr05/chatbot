...
// Setup multer for handling STT audio in memory
const upload = multer({
...
/**
 * Helper: maps various language strings to the precise Sarvam language code (e.g. 'en-IN').
 */
...
/**
 * POST /api/chat
 * Handles chat prompts via Groq and returns response + sentiment + detected language.
 */
  ...
  try {
    ...
    if (!groqKey) {
      return res.status(400).json({
        ...
        message: "GROQ_API_KEY is not configured. Please add it to your Vercel environment variables.",
      });
    }
...
CRITICAL: Your entire reply must be formatted strictly as a single, valid JSON object. Do NOT put your JSON in markdown code blocks. Return a pure raw JSON string ONLY.
...
Keep your responses conversational, engaging, and supportive.`;
 ...
 */
app.post("/api/stt", upload.single("file"), async (req: any, res: any) => {
  try {
...
/**
 * POST /api/tts
 * Synthesizes text to voice using Sarvam AI Bulbul.
 */
